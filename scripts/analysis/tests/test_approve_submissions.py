...
class TestThankYouChaining:
    ...
    def test_send_thank_you_to_pids_dedup_input(self):
        """send_thank_you_to_pids deduplicates the pids list, preserving order."""
        ...
        with (
            _patch.object(sty_module, "prolific_submissions", return_value=mock_subs),
            ...
            # Pass PID_A twice — it should only be messaged once
            stats = send_thank_you_to_pids(["PID_A", "PID_B", "PID_A"], "S1", "token", dry_run=False)

        assert stats["sent"] == 2
        assert sent_to.count("PID_A") == 1
        assert sent_to.count("PID_B") == 1
