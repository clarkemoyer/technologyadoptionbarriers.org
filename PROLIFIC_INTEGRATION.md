...
Currently, no official Prolific MCP (Model Context Protocol) server exists. The TABS project maintains a policy of adopting only official or well-audited community projects for infrastructure. Status is tracked in the [dependency provenance issue #783](https://github.com/clarkemoyer/technologyadoptionbarriers.org/issues/783).

We actively monitor Prolific for an official MCP release. Until such a release occurs, the custom Python scripts in `scripts/analysis/` and the TypeScript API client (`src/lib/prolific-api.ts`) remain the primary methods for programmatic Prolific operations.
