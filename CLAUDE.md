...
**Local npx server setup (Local-dev fallback)** (Claude Desktop — `claude_desktop_config.json`):
...
**Important**: The built-in Copilot plugin injects its own token that cannot be overridden. It is read-only by design. Use the Platform endpoint for the Copilot agent, or the local npx server / `gh` CLI for all write operations.
...
| ------------ | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| **Low**      | Official, from the company, actively maintained          | Qualtrics MCP (First-party), GitHub MCP (Platform), Cloudflare MCP                              |
...
| **Critical** | Missing, deprecated, or cannot verify                    | `@modelcontextprotocol/server-github` (deprecated; use Platform endpoint or local-dev fallback) |

**Rule**: Only adopt from official sources or well-audited community projects.

We actively track Prolific and Qualtrics (SAP) for official MCP server releases:

- **Prolific**: No official MCP exists; currently using custom Python scripts (`scripts/analysis/`) and the TypeScript API client (`src/lib/prolific-api.ts`) for operational tasks.
- **Qualtrics/SAP**: Monitor official SAP/Qualtrics channels for expanded GA support; SAP is a member of the [AI Alliance](https://theaialliance.ai/). We currently use the first-party platform MCP endpoint.
