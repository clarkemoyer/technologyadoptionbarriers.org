...
export type TopBarriersData = {
  top3_pick_counts?: Top3Block
  item_descriptives?: ItemBlock
  extended_last_updated?: string
  last_updated?: string
}

export type TopBarriersVariant = 'live' | 'crp'

interface TopBarriersContentProps {
  variant: TopBarriersVariant
...
const VARIANT_COPY = {
  live: {
    title: 'Top 3 Barriers (Live)',
    jsonFile: 'src/data/sensitivity-analysis.json',
    sourceProse: (
      <>
        which is regenerated daily by the TABS analysis pipeline from the live Prolific Accepted
        dataset (target N=500). The pipeline script is{' '}
        <code className="text-sm bg-gray-100 px-1 py-0.5 rounded">
          scripts/analysis/tabs_v2_unified_data_analysis.py
        </code>
        . Unlike the frozen CRP 2026 snapshot, these numbers change day to day as new responses are
        accepted.
      </>
    ),
  },
  crp: {
    title: 'CRP 2026: Top 3 Barriers',
    jsonFile: 'src/data/crp-sensitivity-analysis.json',
    sourceProse: (
      <>
        which is regenerated daily by the TABS analysis pipeline from the frozen CRP N=200 CSV. The
        pipeline script is{' '}
        <code className="text-sm bg-gray-100 px-1 py-0.5 rounded">
          scripts/analysis/tabs_v2_unified_data_analysis.py
        </code>
...
export function TopBarriersContent({ variant, data }: TopBarriersContentProps) {
  const top3: Top3Block = data.top3_pick_counts ?? {}
  const items: ItemBlock = data.item_descriptives ?? {}

  const lastUpdated: string | null =
    typeof data.extended_last_updated === 'string'
      ? data.extended_last_updated
  ...
  const totalN: number | null = typeof top3.total_n === 'number' ? top3.total_n : null
  ...
  const allPicksUsed: boolean =
    totalN !== null && totalRecordedPicks > 0 && totalRecordedPicks === totalN * 3
  ...
  return (
    <div className="pt-20 sm:pt-[120px] bg-white">
      <article className={ARTICLE_CLASSES}>
        <h1 className={H1_CLASSES}>{copy.title}</h1>
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          ...
          <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-800 border border-gray-200">
            N={totalN !== null ? totalN : DATA_UNAVAILABLE}
          </span>
          <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-amber-50 text-amber-900 border border-amber-200">
            Picks:{' '}
            {totalN !== null && pickSorted.length > 0
              ? `${totalRecordedPicks}${allPicksUsed ? ` (= 3 × ${totalN})` : ''}`
              : DATA_UNAVAILABLE}
          </span>
        </div>
          ...
          {variant === 'live' ? (
            <>
              <p className={PARAGRAPH_CLASSES}>
                ...
                {totalN !== null ? totalN : DATA_UNAVAILABLE}). The numbers update daily as new
                responses are accepted on Prolific. Each participant selected up to three barriers
                from the full list of 18 substantive items. Counts below reflect the number of
                participants who included each barrier in their top 3.
              </p>
              <p className={PARAGRAPH_CLASSES}>
            ...
            <>
              <p className={PARAGRAPH_CLASSES}>
                ...
                frozen dataset (N={totalN !== null ? totalN : DATA_UNAVAILABLE}). Each participant
                selected up to three barriers from the full list of 18. Counts below reflect the
                number of participants who included each barrier in their top 3.
              </p>
              <p className={PARAGRAPH_CLASSES}>
                The forced-choice ranking complements the continuous Likert-based ranking shown on
                the{' '}
                <Link
                ...
                page. Comparing the two rankings surfaces which barriers rise or fall in salience
                when participants are forced to choose only a small number of items rather than rate
                every item on a continuous scale.
              </p>
            </>
          )}
        </section>
          ...
          <section className={SECTION_CLASSES}>
            <div className="border border-amber-200 bg-amber-50 text-amber-900 rounded-lg p-4">
              ...
              <p className="text-sm">
                ...
                written to{' '}
                <code className="text-xs bg-white/50 px-1 py-0.5 rounded">{copy.jsonFile}</code> by
                the most recent pipeline run. Check the latest daily pipeline run and rerender once
                the extended blocks are present.
              </p>
            </div>
          </section>
        )}
        ...
        <section className={SECTION_CLASSES}>
          ...
          {dataAvailable && totalN !== null && (
            <div className="mb-4 border border-amber-200 bg-amber-50 text-amber-900 rounded-lg p-4 text-sm">
              ...
              <p>
                Each of the {totalN} participants could pick up to three barriers, so the Pick N
                column sums across all 18 items to {totalRecordedPicks} recorded picks in this
                dataset. Any single barrier&apos;s Pick N is bounded by {totalN} (the maximum, if
                ...
                {totalN} because participants can contribute more than one pick across their top 3
                choices.
                {allPicksUsed &&
                  ` This equals the maximum possible total of 3 × ${totalN} = ${totalN * 3}.`}
              </p>
            </div>
          )}
          {topPick.length === 0 ? (
        ...
        <section className={SECTION_CLASSES}>
          ...
          <p className={PARAGRAPH_CLASSES}>
            ...
            sums to at most 3N (here 3 x {totalN !== null ? totalN : DATA_UNAVAILABLE} ={' '}
            {totalN !== null ? totalN * 3 : DATA_UNAVAILABLE}).
          </p>
          ...
          <p className={PARAGRAPH_CLASSES}>
            ...
            <code className="text-sm bg-gray-100 px-1 py-0.5 rounded">{copy.jsonFile}</code>,{' '}
            {copy.sourceProse}
          </p>
        </section>
      </article>
