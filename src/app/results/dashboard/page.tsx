        ...
        <article className={ARTICLE_CLASSES}>
                    ...
                    <div key={disposition} className={`border rounded-lg p-4 ${config.bgColor}`}>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className={`text-lg font-bold ${config.color}`}>
                          {count}{' '}
                          <span className="text-sm font-normal text-gray-500">({pct}%)</span>
                        </div>
                      </div>
          ...
          <section className="mb-12">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              {[
                ...
                const pct =
                  autoExcludeTotal > 0 ? ((count / autoExcludeTotal) * 100).toFixed(0) : '0'
                return (
                  ...
                  {[
                    ...
                    .map((row) => (
                      <tr key={row.label} className="border-b border-gray-100">
                        ...
                        <td className={`py-3 px-4 text-right font-bold ${row.color}`}>
                          {row.value}
                        </td>
                      </tr>
                    ))}
          ...
          <section className="mb-12">
            ...
            <div className="bg-white border-2 border-amber-400 rounded-lg p-5 my-6">
              ...
              <p className="text-sm text-gray-800">
                <strong>Expected:</strong> After the daily auto-approve workflow runs, all
                Disposition CLEAN participants should have Prolific_Status == APPROVED, making the
                counts converge. Any persistent gap indicates the approve automation failed to send
                commands to Prolific. The gap is currently{' '}
                <strong>
                  {Math.abs((dispositions['CLEAN'] || 0) - (conservativeCleanN ?? 0))}
                </strong>
                , explained by{' '}
                {(dispositionData.dispositionByStatus as Record<string, Record<string, number>>)?.[
                  'FLAG-PARTIAL-STRAIGHTLINING'
                ]?.['APPROVED'] ?? 0}{' '}
                partial-straightlining responses that were manually approved on Prolific but
                excluded from Conservative Clean.
              </p>
            </div>
          </section>
          ...
          <footer className="mt-12 pt-8 border-t border-gray-200 text-sm text-gray-500">
            <p>
              Last updated:{' '}
              {new Date(
                d.updatedAt.replace(/\.\d{4,}/, (m: string) => m.slice(0, 4))
