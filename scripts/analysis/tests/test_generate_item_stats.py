...
class TestGenerateItemStats:
    def test_basic_structure(self, tmp_path):
        ...
        result = generate_item_stats(csv_path, ["conservative_clean"])["conservative_clean"]
    ...
    def test_invalid_sample_key_raises(self, tmp_path):
        ...
        from generate_item_stats import generate_item_stats
        result = generate_item_stats(csv_path, ["invalid_sample"])
        assert "invalid_sample" not in result
    ...
    def test_prolific_accepted_sample(self, tmp_path):
        ...
        from generate_item_stats import generate_item_stats
        result = generate_item_stats(csv_path, ["prolific_accepted"])["prolific_accepted"]
...
class TestCLI:
    ...
    def test_full_run_writes_json(self, monkeypatch, tmp_path):
        ...
        with open(out_path) as f:
            data = json.load(f)
        assert "conservative_clean" in data
        assert "constructs" in data["conservative_clean"]
        assert data["conservative_clean"]["constructs"]["barriers"]["numItems"] == 18
