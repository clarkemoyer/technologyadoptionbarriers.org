...
export interface ParticipantVoiceData {
  as_of: string
  source_question: string
  source_prompt: string
  stats: {
    total_responses: number
    respondents_in_dataset: number
    engagement_rate_pct: number
    substantive_count: number
    substantive_rate_pct: number
    theme_distribution: Record<string, number>
    length_distribution: Record<string, number>
    length_bucket_definitions: Record<string, string>
  }
  quotes: {
    praise: Quote[]
    insight: Quote[]
...
export type ParticipantVoiceVariant = 'live' | 'crp'
...
interface VariantConfig {
  ...
  findingsHref: string
  sampleHref: string
  resultsOverviewHref: string
  resultsOverviewLabel: string
}

const VARIANT_CONFIG: Record<ParticipantVoiceVariant, VariantConfig> = {
  live: {
    ...
    resultsOverviewLabel: '← Back to Results Overview',
  },
  crp: {
    ...
    sampleHref: '/results/crp-2026/sample',
    resultsOverviewHref: '/results/crp-2026',
    resultsOverviewLabel: '← Back to CRP 2026 Overview',
  },
}

interface Props {
  variant: ParticipantVoiceVariant
  data: ParticipantVoiceData
}

export const ParticipantVoiceContent = ({ variant, data }: Props) => {
  const config = VARIANT_CONFIG[variant]
  const { stats, quotes, source_prompt: sourcePrompt, selection_method: selectionMethod } = data
  const totalQuotes =
    ...
    <div className="bg-white pt-20 sm:pt-[120px]">
      <article className={ARTICLE_CLASSES}>
        ...
        <section className="mb-10 text-gray-800">
          <p className="mb-6">
            {config.intro} {stats.total_responses} of {stats.respondents_in_dataset} respondents (
            {stats.engagement_rate_pct}%) wrote something in the optional Q74 feedback box. Some
            wrote a single word; one wrote nearly 700 characters. This page surfaces what they said.
          </p>
        ...
        <section className="mb-12 text-gray-800">
          ...
          <p className="text-sm text-gray-600">
            The full set of {stats.total_responses} responses lives in the public dataset (the{' '}
        ...
        <section className="border-t border-gray-200 pt-8">
          ...
          <ul className="list-disc space-y-1 pl-6 text-sm text-gray-700">
            ...
            <li>
              ...
              — who these {stats.total_responses} voices belong to in aggregate
            </li>
            <li>
              <Link
                href={config.resultsOverviewHref}
                className="text-blue-600 underline hover:text-blue-800"
              >
                {config.resultsOverviewLabel}
              </Link>
            </li>
          </ul>
        </section>
