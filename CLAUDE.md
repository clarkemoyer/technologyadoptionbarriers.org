...
> **Note on project-level Claude settings:** Project-level hooks are configured in a local `.claude/settings.json` file. This file is per-developer and not committed to the repository (`.claude/` is gitignored); create it locally as needed, or use `~/.claude/settings.json` if you prefer the global Claude settings location.
