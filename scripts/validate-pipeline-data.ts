...
export const DispositionSummarySchema = z.object({
  ...
  completionProgress: z.object({
    ...
    // approved is optional for backwards compatibility with older pipeline output
    approved: z.number().int().nonnegative().optional().default(0),
...
/** Format a Zod issue path for display, using "(root)" when the path is empty. */
...
export function validateDispositionSummary(data: unknown): ValidationResult {
  ...
  return {
    ...
    ok: false,
    errors: result.error.issues.map((i) => `  ${formatIssuePath(i.path)}: ${i.message}`),
  }
}
