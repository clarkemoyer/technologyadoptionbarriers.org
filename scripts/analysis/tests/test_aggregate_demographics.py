...
class TestAggregateCategorical:
    ...
    def test_small_cell_merged_into_other(self):
        # 3 "Non-binary" — below min_cell=5, should merge into Other bucket.
        # Since the merged Other total (3) is also < min_cell=5, the Other bucket
        # is also omitted to enforce the min-cell privacy guarantee.
        values = ["Male"] * 10 + ["Female"] * 8 + ["Non-binary"] * 3
        result = _aggregate_categorical(values, min_cell=5)
        labels = {r["label"] for r in result}
        assert "Non-binary" not in labels
        # Other bucket is suppressed because 3 < min_cell=5
        assert "Other / Prefer not to say" not in labels

    def test_other_bucket_omitted_when_below_min_cell(self):
        # When merged "Other" total is < min_cell, the bucket is omitted entirely
        # so that every reported category has count >= min_cell.
        values = ["Male"] * 10 + ["Female"] * 8 + ["Non-binary"] * 2
        result = _aggregate_categorical(values, min_cell=5)
        labels = {r["label"] for r in result}
        assert "Other / Prefer not to say" not in labels
        # Only the two categories >= min_cell are reported
        assert "Male" in labels
        assert "Female" in labels

    def test_other_bucket_shown_when_meets_min_cell(self):
        # When merged "Other" total is >= min_cell, the bucket IS shown.
        values = ["Male"] * 10 + ["Female"] * 8 + ["NB"] * 3 + ["Prefer"] * 3
        result = _aggregate_categorical(values, min_cell=5)
        labels = {r["label"] for r in result}
        # merged other_count = 6 >= min_cell=5, so Other IS shown
    ...
    def test_small_cell_count_absorbed_into_other(self):
        # When only 3 values are in a small cell, they merge into Other but that
        # merged count (3) is below min_cell=5, so Other is omitted entirely.
        values = ["Male"] * 10 + ["Female"] * 8 + ["Non-binary"] * 3
        result = _aggregate_categorical(values, min_cell=5)
        labels = {r["label"] for r in result}
        assert "Other / Prefer not to say" not in labels
        # Verify that only categories with count >= min_cell appear
        for r in result:
            assert r["count"] >= 5
