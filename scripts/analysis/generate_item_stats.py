...
def generate_item_stats(csv_path: str, sample_keys: Optional[List[str]] = None) -> Dict[str, Dict]:
    """Read the enriched CSV and return item-level statistics as a dict keyed by sample.

    Args:
        csv_path: Path to the enriched Qualtrics CSV.
        sample_keys: List of sample definitions to use.

    Returns:
        Dictionary mapping sample keys to dictionaries suitable for JSON serialization.
    """
...
def main() -> None:
    ...
    print(f"Generating item statistics for all samples...")
    data = generate_item_stats(input_csv)
    ...
    print(f"Item stats written: {output_path} with keys: {list(data.keys())}")
