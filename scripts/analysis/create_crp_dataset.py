...
def select_crp_sample(
    ...
    # Guard: Tier 1 + Tier 2 already exceed target — cannot honour exact N.
    # Both tiers are auto-selected in full, so there is no principled way to
    # trim them without changing the tier definitions. Exit with code 2 so the
    # caller can investigate and adjust (e.g. raise target_n or restrict data).
    if len(tier1) + len(tier2) > target_n:
        print(
            f"  ERROR: Tier 1 ({len(tier1)}) + Tier 2 ({len(tier2)}) = "
            f"{len(tier1) + len(tier2)} exceeds target N={target_n}. "
            "Cannot produce an exactly-N dataset without trimming an auto-selected tier. "
            "Raise --target-n or review inclusion criteria.",
            file=sys.stderr,
        )
        sys.exit(2)
