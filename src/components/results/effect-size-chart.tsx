...
const getEffectSizeMagnitude = (d: number) => {
  const absD = Math.abs(d)

  if (absD < 0.2) {
    return {
      label: 'Negligible',
      color: '#94a3b8', // Slate 400 (Negligible)
    }
  }
  ...
  return {
    label: 'Large',
    color: '#f59e0b', // Amber 500 (Large)
  }
}

const getEffectSizeColor = (d: number) => {
  return getEffectSizeMagnitude(d).color
}

const getEffectSizeLabel = (d: number) => {
  return getEffectSizeMagnitude(d).label
}

const EffectSizeChart: React.FC<EffectSizeChartProps> = ({ data, title }) => {
  const chartData = data.map((item) => {
    const df = item.n1 + item.n2 - 2
    const se = Math.sqrt(
      (item.n1 + item.n2) / (item.n1 * item.n2) + (item.d * item.d) / (2 * Math.max(df, 1))
    )
    const margin = 1.96 * se
    return {
      ...item,
      error: [margin, margin] as [number, number],
    ...
    <div className="w-full bg-white border border-gray-200 rounded-lg p-4 sm:p-6 mb-6 shadow-sm">
      ...
      <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 justify-center text-[10px] sm:text-xs text-gray-500 border-t border-gray-50 pt-4">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span>Small (0.2&ndash;0.5)</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded-sm bg-blue-500" />
          <span>Medium (0.5&ndash;0.8)</span>
        </div>
        <div className="flex items-center gap-1.5">
