...
# gh issue create/edit will fail if a label doesn't exist in the repo.
# Use --force so the command is idempotent (create or update, never error).
gh label create "auto-close-eligible" \
  --color "0075ca" \
  --description "Daily report with no critical findings; eligible for auto-close after 24 h" \
  --force
gh label create "auto-closed" \
  --color "e4e669" \
  --description "Daily report automatically closed after 24 h with no critical findings" \
  --force
