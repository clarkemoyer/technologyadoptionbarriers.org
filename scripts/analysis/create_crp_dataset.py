...
def scan_pii(
    ...
    row_offset: int = 4,
    response_id_col: str = "ResponseId",
) -> list[dict]:
    ...
    Note: when called on a selected subset (not the original input CSV), the
    reported ``row_number`` is relative to the subset ordering and does NOT
    correspond to the original CSV row. Use the ``response_id`` field in each
    flag to cross-reference with the original data.
    """
    flags: list[dict] = []
            ...
            for pattern, pattern_type in PII_PATTERNS:
                for match in pattern.finditer(text):
                    ...
                    flags.append({
                        ...
                        "response_id": row.get(response_id_col, ""),
                        "column": col,
...
def write_pii_report(path: Path, flags: list[dict]) -> None:
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "Note: row numbers below are relative to the scanned data's ordering\n"
            "      (selected subset or output file), not the original input CSV.\n"
            "      Use the ResponseId field to cross-reference with the source data.\n"
        )
        f.write("=" * 72 + "\n\n")
        if not flags:
        ...
        for i, flag in enumerate(flags, 1):
            f.write(f"Flag {i}:\n")
            f.write(f"  Row (scanned data): {flag['row_number']}\n")
            f.write(f"  ResponseId: {flag.get('response_id', '')}\n")
...
def deidentify_selected(
    ...
    if verify_flags:
        ...
        for flag in verify_flags:
            print(f"    Row {flag['row_number']} ({flag.get('response_id', 'unknown')}), {flag['column']}: {flag['pattern_type']}")
...
def main() -> int:
    ...
    if missing_columns:
        print(
            "Error: input CSV is missing required enriched/Qualtrics columns: "
            + ", ".join(missing_columns),
            file=sys.stderr,
        )
        print(
            "Please run scripts/analysis/enrich_qualtrics_csv.py first and "
            "then re-run create_crp_dataset.py with the enriched export.",
            file=sys.stderr,
        )
        return 2
