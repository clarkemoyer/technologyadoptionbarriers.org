...
const VALID_DISPOSITION_SUMMARY = {
  ...
  dispositionByStatus: {
    CLEAN: { APPROVED: 93 },
    INCOMPLETE: { RETURNED: 57, 'TIMED-OUT': 4 },
  },
  study: {
    id: 'abc123',
...
describe('validateDispositionSummary', () => {
  ...
  it('fails when studyName is missing', () => {
    const { studyName: _, ...data } = VALID_DISPOSITION_SUMMARY
    const result = validateDispositionSummary(data)
    expect(result.ok).toBe(false)
    expect(result.errors.some((e) => e.includes('studyName'))).toBe(true)
  })

  it('fails when uniqueParticipants is missing', () => {
    const { uniqueParticipants: _, ...data } = VALID_DISPOSITION_SUMMARY
    const result = validateDispositionSummary(data)
    expect(result.ok).toBe(false)
    expect(result.errors.some((e) => e.includes('uniqueParticipants'))).toBe(true)
  })

  it('fails when autoExcludeBreakdown is missing', () => {
    const { autoExcludeBreakdown: _, ...data } = VALID_DISPOSITION_SUMMARY
    const result = validateDispositionSummary(data)
    expect(result.ok).toBe(false)
    expect(result.errors.some((e) => e.includes('autoExcludeBreakdown'))).toBe(true)
  })
  ...
  it('fails when actions.messaged is missing', () => {
    const { messaged: _, ...actionsWithout } = VALID_DISPOSITION_SUMMARY.actions
    const data = { ...VALID_DISPOSITION_SUMMARY, actions: actionsWithout }
    const result = validateDispositionSummary(data)
    expect(result.ok).toBe(false)
    expect(result.errors.some((e) => e.includes('messaged'))).toBe(true)
  })
  ...
  it('fails when iriPassRates.denominator is missing', () => {
    const { denominator: _, ...iriWithout } = VALID_DISPOSITION_SUMMARY.iriPassRates
    const data = { ...VALID_DISPOSITION_SUMMARY, iriPassRates: iriWithout }
    const result = validateDispositionSummary(data)
    expect(result.ok).toBe(false)
  ...
  it('fails when iriPassRates values are outside 0–100 range', () => {
    const data = {
      ...VALID_DISPOSITION_SUMMARY,
      iriPassRates: { ...VALID_DISPOSITION_SUMMARY.iriPassRates, barrier: 101 },
    }
    const result = validateDispositionSummary(data)
    expect(result.ok).toBe(false)
  })
  ...
  it('fails when an unexpected top-level key is present', () => {
    const data = { ...VALID_DISPOSITION_SUMMARY, unexpectedKey: 'value' }
    const result = validateDispositionSummary(data)
    expect(result.ok).toBe(false)
  })

  it('fails for non-object input', () => {
    const nullResult = validateDispositionSummary(null)
    expect(nullResult.ok).toBe(false)
    // Root-level errors should use '(root)' prefix, not an empty prefix
    expect(nullResult.errors.every((e) => !e.startsWith('  :'))).toBe(true)
...
describe('validateSensitivityAnalysis', () => {
  ...
  it('fails for non-object input', () => {
    const nullResult = validateSensitivityAnalysis(null)
    expect(nullResult.ok).toBe(false)
    // Root-level errors should use '(root)' prefix, not an empty prefix
    expect(nullResult.errors.every((e) => !e.startsWith('  :'))).toBe(true)
...
describe('validateDataAudit', () => {
  ...
  it('fails for non-object input', () => {
    const nullResult = validateDataAudit(null)
    expect(nullResult.ok).toBe(false)
    // Root-level errors should use '(root)' prefix, not an empty prefix
    expect(nullResult.errors.every((e) => !e.startsWith('  :'))).toBe(true)
