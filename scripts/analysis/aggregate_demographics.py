...
def _aggregate_categorical(
    ...
    if other_count >= min_cell:
        ...
        # omit it entirely when the merged count is still below min_cell to ensure
        # no reported category has count < MIN_CELL_SIZE.
        result.append({
...
def aggregate_demographics(csv_path: str, min_cell: int = DEFAULT_MIN_CELL_SIZE) -> Dict:
    ...
    return {
        ...
        "privacyNote": (
            ...
            f"individual privacy. If the merged 'Other' total is also fewer than {min_cell}, "
            "that bucket is omitted so that every reported category has at least "
            f"{min_cell} participants. No participant-level data is included."
        ),
        "age": age_data,
