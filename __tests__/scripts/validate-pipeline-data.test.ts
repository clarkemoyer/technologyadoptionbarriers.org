...
describe('validateDispositionSummary', () => {
  ...
  it('passes when completionProgress.approved is absent (optional field defaults to 0)', () => {
    const { approved: _, ...progressWithoutApproved } = VALID_DISPOSITION_SUMMARY.completionProgress
    const data = {
      ...VALID_DISPOSITION_SUMMARY,
      completionProgress: progressWithoutApproved,
    }
  ...
  it('fails for non-object input and errors include "(root)" label', () => {
    const nullResult = validateDispositionSummary(null)
    expect(nullResult.ok).toBe(false)
    expect(nullResult.errors.some((e) => e.includes('(root)'))).toBe(true)
    expect(nullResult.errors.every((e) => !e.match(/^\s*:/))).toBe(true)
