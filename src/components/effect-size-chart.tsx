...
const EffectSizeChart: React.FC<EffectSizeChartProps> = ({ data, title }) => {
  ...
  let max = 0
  chartData.forEach((d) => {
    if (d.d === null || d.d === undefined) return
  ...
  return (
    <figure
      ...
      aria-label={!title ? 'Effect Size Bar Chart' : undefined}
    >
      {title && (
        <figcaption
          id={chartTitleId}
          className="text-sm font-semibold text-gray-700 mb-4 text-center"
        >
          {title}
        </figcaption>
      )}
            ...
            <Bar dataKey="d" barSize={32}>
              {chartData.map((entry, index) => (
                <Cell
                  ...
                  fill={entry.fill}
                  radius={
                    entry.d !== null && entry.d !== undefined && entry.d < 0
                      ? [4, 0, 0, 4]
                      : [0, 4, 4, 0]
                  }
                />
