...
def main():
    ...
    try:
        ...
        if dry_run:
            ...
            for pid in clean_pids:
                status = current_statuses.get(pid, "UNKNOWN")
                # Only message participants who are APPROVED or about to be approved
                if status not in ("APPROVED", "AWAITING REVIEW"):
                    continue
                try:
                    # Check for existing thank-you message (dedupe by SIGNATURE + study_id)
    ...
    _write_step_summary(study_id, dry_run, total_data_rows, len(clean_pids), skipped,
                        already_approved, newly_approved, messages_sent, messages_already_sent,
                        messages_failed)
