...
export const getEffectSizeMagnitude = (d: number | null | undefined): EffectSizeMagnitude => {
  if (d === null || d === undefined || !Number.isFinite(d)) return 'missing'
...
const EffectSizeChart: React.FC<EffectSizeChartProps> = ({ data, title }) => {
  const chartData = data.map((item) => {
    ...
    if (hasValue) {
      // Standard error formula for Cohen's d: sqrt((n1+n2)/(n1*n2) + d^2 / (2*(n1+n2-2)))
      const se = Math.sqrt(
        (item.n1 + item.n2) / (item.n1 * item.n2) + (d * d) / (2 * (item.n1 + item.n2 - 2))
      )
      margin = 1.96 * se
    }
    ...
    return {
      ...
      d,
      hasValue,
      error: [margin, margin] as [number, number], // [lower, upper] relative to d
            ...
            <Bar dataKey="d" barSize={20}>
              {chartData.map((entry, index) => {
                const radius: [number, number, number, number] =
                  entry.d < 0 ? [4, 0, 0, 4] : [0, 4, 4, 0]
                return <Cell key={`cell-${index}`} fill={entry.color} radius={radius} />
              })}
