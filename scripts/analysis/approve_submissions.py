...
def main():
    ...
    try:
        ...
        if dry_run:
            ...
            for pid in clean_pids:
                status = current_statuses.get(pid, "UNKNOWN")
                # Message participants that are APPROVED or AWAITING REVIEW (the latter
                # were just approved in this run; current_statuses was fetched before
                # prolific_bulk_approve, so their cached status may not yet reflect APPROVED)
