...
Project-level hooks can be defined in `.claude/settings.json` (gitignored — local only). Because `.claude/` is not committed to the repo, these hooks are an **optional per-developer convenience** and are not available to CI or other contributors by default.
...
**Important**: Hooks only run if you have a local `.claude/settings.json` with them configured. Contributors without this file must still run `npm run format` manually before committing — CI enforces `npm run format:check` and will reject PRs with formatting errors. Hooks are a convenience layer on top of the required npm scripts, not a replacement for them.
